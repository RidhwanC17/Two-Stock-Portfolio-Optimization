...
median_returns = train_returns.median() * 252

mad = (train_returns - train_returns.median()).abs().median()

robust_std = 1.4826 * mad * np.sqrt(252)

risk_free_rate = 0.05
corr = train_returns.corr()

robust_cov_matrix = np.outer(robust_std, robust_std) * corr

mu = median_returns.values
Sigma = robust_cov_matrix
